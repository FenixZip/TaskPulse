...
def test_task_mark_overdue_sets_status_and_logs(task_factory):
    ...
    task = task_factory(due_at=timezone.now() - timedelta(hours=1),
...
def test_task_save_logs_priority_status_due_changes(task_factory):
    ...
    task.save()
    assert TaskChangeLog.objects.filter(task=task, field="priority",
                                        old_value=old_p,
                                        new_value=Task.Priority.HIGH).exists()
    old_s = task.status
    task.status = Task.Status.IN_PROGRESS
    task.save()
    assert TaskChangeLog.objects.filter(task=task, field="status",
                                        old_value=old_s,
                                        new_value=Task.Status.IN_PROGRESS).exists()
    # меняем срок
    old_due = task.due_at
    task.due_at = timezone.now() + timezone.timedelta(days=3)
    task.save()
